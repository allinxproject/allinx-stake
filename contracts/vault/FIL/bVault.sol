...
contract iVault is ERC20, ERC20Detailed {
    ...
    mapping(address => uint256) userTotalDepositAmount;
    address public feeAddress;
    address public redeliveryAccount;

    uint public redeliveryRatio = 300;
    ...
    constructor (address _token,address _playerBook) public ERC20Detailed(
        string(abi.encodePacked("inx:vault: ", ERC20Detailed(_token).name())),
        ...
        feeAddress = 0xB0E76C7081267A8b7259C42AffD5b0A48D236DE4;
        redeliveryAccount = 0x2817a87E7e43D42F146dd93942f8604467727733;
		timelock = tx.origin;
    ...
    function withdraw(uint _shares) public onlyRestrictContractCall {
        ...
        uint256 _withdrawBaseAmount = _shares.mul(userTotalDepositAmount[msg.sender]).div(balanceOf(msg.sender));
        userTotalDepositAmount[msg.sender] = userTotalDepositAmount[msg.sender].sub(_withdrawBaseAmount);
        ...
        if(r > _withdrawBaseAmount){
            uint256 _fee = r.sub(_withdrawBaseAmount).mul(redeliveryRatio).div(feeMax);
            r = r.sub(_fee);
            token.safeTransfer(redeliveryAccount,_fee);
        }
    ...
    function setRedeliveryRatio(uint256 _ratio) public
    {
        require(msg.sender == timelock, "!timelock");
        require(_ratio <= 500,"The fee is too high");
        redeliveryRatio = _ratio;
    }
    ...
    function setRedeliveryAccount(address _redeliveryAccount) public
    {
        require(msg.sender == timelock, "!timelock");
        redeliveryAccount = _redeliveryAccount;
    }
