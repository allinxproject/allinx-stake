...
contract StrategyWBNBAuto {
    ...
    constructor() public {
        ...
        doApprove();
		swap2WantRouting = [AUTOv2,wbnb];
        swap2TokenRouting = [wbnb,usdt];
        swap2INXRouting = [usdt,inx];
    }

	function doApprove () internal{
        IERC20(AUTOv2).approve(pancakeRouter, uint(-1));
        IERC20(wbnb).approve(pancakeRouter, uint(-1));
	...
	function setSwapRouting(uint _index,address[] memory _routing)public {
		...
		if(_index==0){
            ...
            IERC20(_routing[0]).approve(pancakeRouter, uint(-1));
		} else {
    ...
    function redelivery() internal {
        ...
        if (reward > redeliverynum){
            uint256 _wbnbBefore = IERC20(wbnb).balanceOf(address(this));
            PancakeRouter(pancakeRouter).swapExactTokensForTokens(reward, 0, swap2WantRouting, address(this), now.add(1800));
            uint256 _wbnbAfter = IERC20(wbnb).balanceOf(address(this));
            uint256 _2usdt = _wbnbAfter.sub(_wbnbBefore).mul(100-redeliveryRate).div(100);
            PancakeRouter(pancakeRouter).swapExactTokensForTokens(_2usdt, 0, swap2TokenRouting, address(this), now.add(1800));
            uint256 _2inx = IERC20(usdt).balanceOf(address(this));
