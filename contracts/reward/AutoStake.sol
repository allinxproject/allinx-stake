...
interface IMintRewardPool {
  ...
  function stake(uint256 amount, string calldata affCode) external;
  function earned(address account) external view returns (uint256);
}

import "../library/Governance.sol";

contract AutoStake is Governance {
  using SafeERC20 for IERC20;
  using SafeMath for uint256;

  address public teamWallet = 0x5D11DaD5373511a625B1d99E219Ec6711932F3B5;
  address public punishWallet = 0xB0E76C7081267A8b7259C42AffD5b0A48D236DE4;

  uint256 public teamRewardRate = 500;
  uint256 public punishRewardRate = 7000;
  ...
  event Withdrawn(address indexed user, uint256 total,uint256 reward, uint256 fee);
  ...
  function exit() public {
    ...
    uint256 reward = toTransfer > stakedBalance[msg.sender] ? toTransfer.sub(stakedBalance[msg.sender]) : 0;
    uint256 fee = punishRewardFee(reward);
    lpToken.safeTransfer(msg.sender, toTransfer.sub(fee));
    totalShares = totalShares.sub(share[msg.sender]);
    share[msg.sender] = 0;
    stakedBalance[msg.sender] = 0;
    emit Withdrawn(msg.sender, toTransfer,reward,fee);
    // Rate needs to be updated here, otherwise the valuePerShare would be incorrect.
    updateValuePerShare();
  ...
  function punishRewardFee(uint256 reward) internal returns (uint256){
    uint256 fee = reward.mul(teamRewardRate).div(baseRate);
    if(fee>0){
      lpToken.safeTransfer(teamWallet, fee);
    }
    uint256 leftReward = reward.sub(fee);
    //withdraw time check

    if(now  < (lastStakedTime[msg.sender] + punishTime) ){
      uint256 punishReward = leftReward.mul(punishRewardRate).div(baseRate);
      if(punishReward>0){
        lpToken.safeTransfer(punishWallet, punishReward);
        fee = fee.add(punishReward);
      }
    }
    return fee;
  }
  ...
  function setTeamRewardRate( uint256 teamRate ) public onlyGovernance{
    require(teamRate <= 2000, "teamRate <= 2000");
    teamRewardRate = teamRate;
  }

  function setPunishRewardRate( uint256  punishRate ) public onlyGovernance{
    require(punishRate <= 7000, "punishRate <= 7000");
    punishRewardRate = punishRate;
  }

  function setWithDrawPunishTime( uint256 _punishTime ) public onlyGovernance{
    require(_punishTime <= 15 days, 'punishTime <= 15 days');
    punishTime = _punishTime;
  }
}